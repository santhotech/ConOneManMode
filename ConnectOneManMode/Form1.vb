...
Public Class Form1
    ...
    Sub DisableAll()
        Dim allTxt As New List(Of Control)
        For Each btn As Button In FindControlRecursive(allTxt, Me, GetType(Button))
            AccessControlBtn(btn, 0)
        Next
    End Sub
    ...
    Sub PopulateTextBox(ByVal strmArr() As String)
        Dim len As Integer = strmArr.Length
        For i = 0 To len - 1
            AccessControlTxt(TextBox3, strmArr(i))
        Next
    End Sub
    ...
    End Sub
    Private Delegate Sub AccessControlBtnDelegate(ByVal btn As Button, ByVal i As Integer)
    Private Sub AccessControlBtn(ByVal btn As Button, ByVal i As Integer)
        If Me.InvokeRequired Then
            ...
            Me.Invoke(delegate2, parameters)
        Else
            If i = 1 Then
                btn.Enabled = True
            ElseIf i = 0 Then
        ...
        End If
    End Sub
    Private Delegate Sub AccessControlTxtDelegate(ByVal txt As Control, ByVal str As String)
    Private Sub AccessControlTxt(ByVal txt As Control, ByVal str As String)
        If Me.InvokeRequired Then
            ...
            parameters(1) = str
            'Ask form me to call this sub using the delegate with the parameters.
            Me.Invoke(delegate3, parameters)
        Else
        ...
        End If
    End Sub
    Private Delegate Sub AccessControlRadioDelegate(ByVal rdo As RadioButton)
    Private Sub AccessControlRadio(ByVal rdo As RadioButton)
        If Me.InvokeRequired Then
            Dim delegate4 As New AccessControlRadioDelegate(AddressOf AccessControlRadio)
            Dim parameters(0) As Object
            parameters(0) = rdo
            'Ask form me to call this sub using the delegate with the parameters.
            Me.Invoke(delegate4, parameters)
        Else
            rdo.Checked = True
        End If
    End Sub
